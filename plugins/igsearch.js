...
// إعداد الأمر ليتوافق مع Kakachi-bot
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`*⚠️ يرجى إدخال نص البحث*\nمثال: \n${usedPrefix + command} كريستيانو رونالدو`);

  m.reply('*🔄 جاري البحث وجلب الريلز المتاحة...*');

  try {
    const searchTarget = `${encodeURIComponent(text)} site:://instagram.com`;
    let html = await fetchText(`${SEARCH_URL}?q=${searchTarget}`).catch(() => null);
    let results = html ? extractSearchResults(html, MAX_RESULTS) : [];

    if (!results.length) {
      html = await fetchText(`${DUCKDUCKGO_URL}?q=${searchTarget}`).catch(() => null);
      results = html ? extractDuckDuckGoResults(html, MAX_RESULTS) : [];
    }

    if (!results.length) return m.reply('*❌ لم يتم العثور على أي نتائج مطابقة في إنستغرام.*');

    // أخذ أول نتيجة فقط وتحميلها كفيديو
    const bestResult = results[0];
    const media = await scrapeInstagramPage(bestResult.permalink);

    if (!media.videoUrl) return m.reply('*❌ تعذر الحصول على رابط تحميل مباشر للفيديو.*');

    let response = await fetch(media.videoUrl, { headers: { "User-Agent": USER_AGENT } });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
